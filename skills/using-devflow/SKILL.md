...
| `executing-plans` | "Execute the plan", "Start implementing" | Serial milestone execution with checkpoint reviews |
| `finishing-work` | Invoked by executing-plans | Test verification → merge/PR/keep/discard options → cleanup |
| `reviewing-code` | "Review my code", "/review", "Check this PR" | Dispatch reviewer → structured report + handle feedback |
...
- **Load on trigger match, before any exploration.** The skill defines its own exploration scope; pre-reading files or spawning subagents first duplicates that work and wastes tokens.
- **`systematic-debugging` and `reviewing-code` trigger independently.** They don't require running the full devflow chain — load them directly when the request is a bug or a review.
- **Don't narrate the chain as a plan.** Loading the first skill and letting it drive *is* the workflow; describing "first I'll brainstorm, then plan…" is stalling.
